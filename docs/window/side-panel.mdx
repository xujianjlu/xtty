...
The right-hand Info / Source Control / Files column is **not** a primary
surface in current xtty builds: it stays off by default, has no title-bar or
View-menu open control, and no default shortcut. Power users can still set
`right_panel_visible: true` in `config.json` or bind `ToggleRightPanel` on the
Keybindings page. When open, the three tab icons switch tabs, and clicking the
lit one (or the panel chrome tile) puts it away again.
...
carry — past that the panel tells you to use [SFTP](/remote/workspaces).
