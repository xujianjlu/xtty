...
| Key | Type | Default | |
|---|---|---|---|
| `prompt_editor` | bool | `false` | Advanced; needs shell integration. When on, tty7 edits the shell prompt. Off (default) sends every key at the prompt to the shell, so its own line editor (ZLE, readline) owns editing. Shell integration stays on. |
