...
| Key | Type | Default | |
|---|---|---|---|
| `prompt_editor` | bool | `false` | Advanced; needs shell integration. When on, tty7 edits the shell prompt. Off (default) sends ordinary keys at the prompt to the shell's own line editor (ZLE, readline). Shell integration stays on. <kbd>⌃ R</kbd> / <kbd>⌘ R</kbd> still follow `history_search`. |
| `tab_completion` | bool | `true` | tty7's completion menu on <kbd>⇥</kbd>. Off hands the key to the shell. Needs `prompt_editor`. |
| `history_search` | bool | `true` | tty7's fuzzy history on <kbd>⌃ R</kbd> or <kbd>⌘ R</kbd>. Independent of `prompt_editor`; also works after an inner SSH/jumper/su where shell integration is unavailable. Off hands the key to the shell. |
