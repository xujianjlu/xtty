...
Nested `ssh` hops (local → jumper → dev box) do not re-inject integration into
inner shells. Tab chips still track the *current* machine via the dialled
identity seed, process-table / OSC 133;C hop detection, OSC 0 `user@host`, and
OSC 7 host retargeting — they do not fall through to cwd paths.
