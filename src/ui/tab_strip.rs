...
impl TabChipOptions {
    pub(crate) fn from_config(cfg: &crate::core::config::Config) -> Self {
        Self {
...
pub(crate) fn label_of(
    ...
    label_of_with(view, index, home, TabChipOptions::default())
}

pub(crate) fn label_of_with(
    view: &crate::ui::machine_mirror::TabView,
    index: usize,
    home: Option<&std::path::Path>,
    ...
    let basename_of = |raw: &str| -> Option<String> {
        if !options.show_cwd_basename {
            return None;
        }
        let s = shortened(raw);
    ...
    match view.label() {
        ...
        TabLabel::Cwd(cwd) => match basename_of(cwd) {
            Some(leaf) => leaf,
            // Basename display off: still name the tab somehow.
            None if options.show_user_host => match view
                .osc_title
                .as_deref()
                .and_then(tty7_core::core::tab_view::identity_from_title)
            {
...
pub(crate) fn tooltip_of_with(
    ...
    let label = label_of_with(view, index, home, options);
...
impl Tty7App {
    ...
    pub(crate) fn tab_title_tooltip(
        ...
        let options = TabChipOptions::from_config(cx.global::<Config>());
        tooltip_of_with(&view, index, home.as_deref(), options)
    }
    ...
    pub(crate) fn tab_label(
        ...
        let (view, home) = tab.label_view(window, cx);
        let options = TabChipOptions::from_config(cx.global::<Config>());
        label_of_with(&view, index, home.as_deref(), options)
    }
...
mod ssh_host_row_tests {
    ...
    use crate::terminal::view::{quiet_test_pane, quiet_test_ssh_hop_pane, quiet_test_ssh_pane};
    ...
    fn only_a_tab_on_an_ssh_host_is_offered_the_host_form(cx: &mut TestAppContext) {
        ...
        let _ends = app.update_in(&mut vcx, |app, window, cx| {
            ...
            let (dialled, b) = quiet_test_ssh_hop_pane(2, "other@elsewhere", window, cx);
            let (from_host, c) = quiet_test_ssh_pane(3, window, cx);
        ...
        app.update_in(&mut vcx, |app, window, cx| {
            ...
            let (form, label) = app
                ...
                .expect("an in-shell hop with no saved host offers to save it");
            assert_eq!(
                form,
                TabHostForm::Saved(RemoteTarget::direct("other", "elsewhere", 22))
            );
            assert_eq!(label, t(L10nKey::SwitcherSaveAsHost));

            let (form, label) = app
                .tab_ssh_host_form(2, window, cx)
                .expect("a hop matching a saved host offers to edit it");
            assert_eq!(
                form,
    ...
    fn a_host_deleted_under_a_live_tab_is_offered_back_as_a_new_one(cx: &mut TestAppContext) {
        ...
        let _end = app.update_in(&mut vcx, |app, window, cx| {
            let (view, end) = quiet_test_ssh_pane(1, window, cx);
            app.tabs.push(Tab::new(Pane::leaf(PaneSlot::Ready(view))));
            app.active = 0;
            cx.notify();
            end
        ...
        app.update_in(&mut vcx, |app, window, cx| {
            let (form, label) = app
                ...
                .expect("an unmatched hop still names a host worth keeping");
            assert_eq!(
                form,
                TabHostForm::Saved(RemoteTarget::direct("me", "build-box", 22))
            );
            assert_eq!(label, t(L10nKey::SwitcherSaveAsHost));
        });
    }
...
mod tests {
    ...
    /// Settings → Appearance → Tabs: turning `show_user_host` back on composes
    /// `user@host:basename` instead of dropping identity to the tooltip only.
    #[test]
    fn tab_chip_options_can_put_user_host_beside_basename() {
        let mut tab = strip_tab();
        tab.osc_title = Some("deploy@box".into());
        tab.cwd = Some("/Users/x/repo/app".into());

        assert_eq!(
            label_of_with(&tab, 0, Some(home()), TabChipOptions::default()),
            "app"
        );
        assert_eq!(
            label_of_with(
