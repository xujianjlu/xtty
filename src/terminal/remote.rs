...
impl RemoteTerminal {
    ...
    pub(super) fn from_stream_reattached(stream: Stream, size: TermSize) -> anyhow::Result<Self> {
        Self::from_stream_parts(stream, size, Vec::new(), true)
    }
    ...
    pub(super) fn from_stream_with(
        ...
        Self::from_stream_parts(stream, size, buffered, false)
    }
...
fn connect_routed(route: &PaneRoute) -> anyhow::Result<Stream> {
    ...
    // No duplicate WSL/server probe here: the daemon runs the same probe inside
    // `router::open_link` before it opens the link.
    let mut stream = connect()?;
...
fn terminal_config_from_user(user_config: &crate::core::config::Config) -> Config {
    Config {
        ...
        kitty_keyboard: true,
        // macOS-only GUI: never a local ConPTY. Keep the field wired so a
        // future Windows client revive does not silently drop the resize fix.
