...
- **Cursor blink interval is configurable.** Settings → Appearance → Cursor
  gained a stepper for how long each on/off phase lasts (100–2000 ms, default
  530), and the same value is the `cursor_blink_interval_ms` key in
  `config.json`. The terminal caret and the home-page caret share the clock, so
  turning the interval up slows both without restarting.
